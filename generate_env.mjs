...
const servicePorts = {
  ...
  'super-communication-service': 5028
};
  ...
  services.forEach(service => {
    ...
    if (fs.statSync(servicePath).isDirectory() && servicePorts[service]) {
      ...
      if (service === 'business-dashboard-service') {
        extra = `MARKETPLACE_SERVICE_URL=http://localhost:5008\nRIDE_SERVICE_URL=http://localhost:5009\nFOOD_SERVICE_URL=http://localhost:5010\nHOTEL_SERVICE_URL=http://localhost:5021\nORDER_SERVICE_URL=http://localhost:5036`;
      }
      if (service === 'global-search-service') {
        extra = `USER_SERVICE_URL=http://localhost:5002\nSOCIAL_SERVICE_URL=http://localhost:5004\nADVANCED_MARKETPLACE_SERVICE_URL=http://localhost:5018\nMARKETPLACE_SERVICE_URL=http://localhost:5008\nHOTEL_SERVICE_URL=http://localhost:5021\nFOOD_SERVICE_URL=http://localhost:5010\nPROFESSIONAL_SERVICE_URL=http://localhost:5006`;
      }
