...
const signup = async (req, res, next) => {
  try {
    ...
    // Sync with User-Service (Profile Creation)
    try {
      // In this specific monolithic/microservice hybrid, we know where User-Service is
      // This is a "Distributed Transaction" simplified for the demo
      const userApi = process.env.USER_SERVICE_URL || 'http://localhost:5002';
      await fetch(`${userApi}/profile`, {
        method: 'POST',
      ...
      }).catch(err => console.error('Profile sync failed:', err.message));
    } catch (e) {}
