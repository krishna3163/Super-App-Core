      ...
      <div className="p-8">
        ...
        <div className="space-y-3">
          {transactions.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-gray-900 rounded-[2rem] border-2 border-dashed border-gray-100 dark:border-gray-800">
               <p className="text-xs font-black text-gray-400 uppercase tracking-widest">No transactions yet</p>
            </div>
          ) : (
            transactions.map(tx => (
              <div key={tx._id} className="bg-white dark:bg-gray-900 p-5 rounded-[2rem] flex items-center gap-4 shadow-sm border dark:border-gray-800 hover:scale-[1.02] transition-transform">
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm p-4">
              <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-[3rem] p-8 pb-12 shadow-2xl animate-in slide-in-from-bottom duration-300">
                  <div className="flex justify-between items-center mb-8">
                      <h2 className="text-xl font-black uppercase tracking-widest text-slate-800 dark:text-white">Top Up Wallet</h2>
                      <button onClick={() => setShowTopUp(false)} className="p-2 bg-slate-100 dark:bg-gray-800 rounded-full"><X size={20}/></button>
                  </div>
