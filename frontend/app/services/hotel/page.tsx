...
const HOTELS = [
  { _id: 'h1', name: 'The Grand Palace', address: 'Connaught Place, Delhi', rating: 4.8, reviewCount: 1240, image: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600&q=80', amenities: ['WiFi', 'Pool', 'Gym', 'Parking', 'Restaurant'], ownerId: 'grand_palace',
    rooms: [
      { _id: 'r1', type: 'Deluxe Room', price: 3500, maxGuests: 2, image: 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=400&q=80', desc: 'King bed, city view, AC' },
      { _id: 'r2', type: 'Suite', price: 7500, maxGuests: 4, image: 'https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?w=400&q=80', desc: 'Living room, jacuzzi, panoramic view' },
      { _id: 'r3', type: 'Standard Room', price: 2200, maxGuests: 2, image: 'https://images.unsplash.com/photo-1618773928121-c32242e63f39?w=400&q=80', desc: 'Queen bed, garden view' },
    ]},
  { _id: 'h2', name: 'Seaside Resort', address: 'Marine Drive, Mumbai', rating: 4.6, reviewCount: 890, image: 'https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?w=600&q=80', amenities: ['WiFi', 'Beach', 'Spa', 'Restaurant'], ownerId: 'seaside_resort',
    rooms: [
      { _id: 'r4', type: 'Sea View Room', price: 5500, maxGuests: 2, image: 'https://images.unsplash.com/photo-1571003123894-1f0594d2b5d9?w=400&q=80', desc: 'Direct sea view, balcony' },
      { _id: 'r5', type: 'Beach Villa', price: 12000, maxGuests: 6, image: 'https://images.unsplash.com/photo-1602002418082-a4443e081dd1?w=400&q=80', desc: 'Private beach access, pool' },
    ]},
]
...
export default function HotelPage() {
  ...
  const placeBooking = async () => {
    ...
    setPlacing(true)
    try {
      const { data } = await api.post('/hotel/book', {
  ...
  const sendChat = async () => {
    ...
    setOrderChatMsg('')
    // In real app: api.post(`/hotel/bookings/${booked._id}/chat`, msg)
  }
      ...
      <div className="p-5 space-y-5">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <div className="flex items-center gap-1 text-yellow-500 text-sm font-black"><Star size={14} fill="currentColor"/>{selected.rating}</div>
            <span className="text-gray-400 text-xs">({selected.reviewCount} reviews)</span>
            <span className="flex items-center gap-1 text-gray-400 text-xs"><MapPin size={11}/>{selected.address}</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {selected.rooms.map((room: any) => (
            <button key={room._id} onClick={() => { setSelectedRoom(room); setShowBooking(true) }}
              ...
              <img src={room.image} className="w-20 h-20 rounded-2xl object-cover shrink-0" alt=""/>
      ...
      <div className="p-4 space-y-4">
        {HOTELS.map(hotel => (
          <button key={hotel._id} onClick={() => setSelected(hotel)} className="w-full bg-white dark:bg-gray-900 rounded-[2rem] overflow-hidden border dark:border-gray-800 shadow-sm hover:shadow-lg transition-all text-left group">
            <div className="h-44 overflow-hidden"><img src={hotel.image} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt=""/></div>
            <div className="p-4">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-black text-base dark:text-white">{hotel.name}</h3>
                  <p className="text-xs text-gray-400 flex items-center gap-1 mt-0.5"><MapPin size={10}/>{hotel.address}</p>
                </div>
              ...
              <div className="flex gap-1.5 mt-3 flex-wrap">
                {hotel.amenities.slice(0, 4).map(a => <span key={a} className="text-[10px] bg-gray-100 dark:bg-gray-800 text-gray-500 px-2 py-1 rounded-lg font-bold">{a}</span>)}
              </div>
              <div className="flex justify-between items-center mt-3 pt-3 border-t dark:border-gray-800">
                <p className="text-xs text-gray-400">From <span className="font-black text-blue-600 text-base">₹{Math.min(...hotel.rooms.map(r => r.price)).toLocaleString()}</span>/night</p>
