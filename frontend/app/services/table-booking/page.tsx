...
export default function TableBookingPage() {
  ...
  if (booked) return (
    <div className="min-h-screen bg-[var(--bg-primary)] pb-24">
      <div className="max-w-lg mx-auto p-5 space-y-5">
        <div className="bg-[var(--bg-card)] rounded-[2rem] p-6 text-center border border-white/5 shadow-xl">
  ...
  if (selected) return (
    <div className="min-h-screen bg-[var(--bg-primary)] pb-32">
      <div className="relative h-52">
        ...
        <button onClick={() => setSelected(null)} className="absolute top-4 left-4 p-2 bg-black/50 backdrop-blur-md rounded-xl text-white border border-white/20"><ChevronLeft size={20}/></button>
      </div>
      ...
      {showBooking && selectedTable && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end justify-center">
          <div className="bg-[var(--bg-card)] rounded-t-[2rem] w-full max-w-lg max-h-[92vh] overflow-y-auto animate-in slide-in-from-bottom-4 border-t border-white/5">
            <div className="p-5 border-b border-white/5 flex justify-between items-center sticky top-0 bg-[var(--bg-card)]/90 backdrop-blur-md z-10">
              <h3 className="font-black text-lg text-[var(--text-primary)]">Book Table</h3>
              <button onClick={() => setShowBooking(false)} className="p-2 bg-[var(--bg-elevated)] rounded-xl"><X size={18}/></button>
            </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] pb-24">
      <div className="px-5 py-4 bg-[var(--bg-card)] border-b border-white/5 sticky top-0 z-20 flex items-center gap-3">
        <Link href="/apps" className="p-2 bg-[var(--bg-elevated)] rounded-xl border border-white/5"><ChevronLeft size={18} className="text-[var(--text-primary)]"/></Link>
        <h1 className="text-xl font-black text-[var(--text-primary)]">Table Booking</h1>
      </div>
