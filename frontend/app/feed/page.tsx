...
export default function FeedPage() {
  ...
  const fetchFeed = async () => {
    try {
      setLoading(true)
      const hashtag = new URLSearchParams(window.location.search).get('hashtag')
      const postId = new URLSearchParams(window.location.search).get('post')
      
      let res;
      if (hashtag) {
        res = await api.get(`/social/search/hashtag?hashtag=${hashtag}`)
      } else if (postId) {
        res = await api.get(`/social/posts/${postId}`)
      } else {
        res = await api.post('/social/feed', { followingIds: [], limit: 50 })
      }
  ...
  const handleDelete = async (postId: string) => {
    ...
    try {
      await api.delete(`/social/posts/${postId}`, { data: { userId: user?.id } })
  ...
  const handleEdit = async (postId: string, newContent: string) => {
    try {
      const res = await api.patch(`/social/posts/${postId}`, { content: newContent, userId: user?.id })
      if (res.data.status === 'success') {
